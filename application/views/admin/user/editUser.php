...
<div class="col-md-6 col-md-offset-3 marginTop60px">
	...
	<?= form_open('adminUser/editUser',['class'=>'form']); ?>
		...
		<?php if($this->session->userdata('COSTUME')['level'] != 'operator') : ?>
		<select name="level" class="form-control">
			<option disabled="" selected="">Level</option>
			<?php 

				if($this->session->userdata('COSTUME')['level'] == 'admin')
				$level = ['operator','admin'];
				elseif($this->session->userdata('COSTUME')['level'] == 'superAdmin')
